...
fn success() {
    ...
    let num_utxos_created = test_create_utxos(&mut wallet, online.clone(), true, None, None);
    ...
    let num_utxos_created =
        test_create_utxos(&mut wallet, online.clone(), true, Some(UTXO_NUM + 1), None);
...
fn up_to_allocation_checks() {
    ...
    if MAX_ALLOCATIONS_PER_UTXO > 2 {
        ...
        let num_utxos_created = test_create_utxos(&mut wallet, online.clone(), true, Some(2), None);
        assert_eq!(num_utxos_created, 2);
        let num_utxos_created =
            test_create_utxos(&mut rcv_wallet, rcv_online.clone(), true, Some(1), None);
        assert_eq!(num_utxos_created, 1);
        // issue
        let asset = wallet
            .issue_asset_rgb20(
        ...
        let num_utxos_created = test_create_utxos(&mut wallet, online, true, Some(4), None);
        assert_eq!(num_utxos_created, 1);
        // UTXO 1 (blind) has at least 1 free allocation
